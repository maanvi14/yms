...
class LocalEmbedder:
    """FREE local embeddings - ChromaDB compatible"""
...
class RAGStore:
    ...
    def __init__(self):
        ...
        print(f"✅ RAGStore ready | LLM: {self.llm_model}")
...
DEFAULT_DOCS = [
    KnowledgeDoc(
        ...
        content="Empty trailers must not exceed 12 hours dwell time. Warning at 10 hours. Zone C monitored hourly due to 87% typical capacity.",
...
def init_knowledge(store: RAGStore):
    ...
    for doc in DEFAULT_DOCS:
        store.add_doc(doc)
    print(f"✅ Initialized {len(DEFAULT_DOCS)} default docs")
